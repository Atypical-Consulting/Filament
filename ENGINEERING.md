# Filament — Engineering Log (Phase 0 baseline + Phase 1 runtime + Phase 2/3 generator)

> **New here?** Start with [`README.md`](./README.md) for the problem/solution overview. This file is the
> full engineering log — every phase, gate, and measurement in context.
...
| `tools/route-contract.mjs` | The **behavioural** gate for a routed app with route parameters (DECISIONS.md #163). `canon` decides the BYTES; this decides what they DO — it bundles the emitted router through esbuild, runs it under happy-dom, and drives 20 steps (precedence, the four converters, URL decoding, instance reuse across a parameter-only navigation). Not redundant with canon: canon's own limitation **L3** makes object-literal keys invisible to it, and the converter table is an object literal. `node tools/route-contract.mjs <dir-with-Router.g.js>`. |
| `tools/error-boundary-contract.mjs` | The **behavioural** gate for `<ErrorBoundary>` (DECISIONS.md #164). `canon` decides the BYTES; this decides what they DO — it bundles the emitted module through esbuild, runs it under happy-dom, and drives 5 steps. **Every step carries a control that must BREAK it**, and a control that cannot fail is reported as INAPPLICABLE rather than counted (two of them were corrected by this rule). `node tools/error-boundary-contract.mjs <emitted.g.js>`. |
| `tools/error-boundary-oracle/` | The **Blazor-side** oracle for `<ErrorBoundary>` (DECISIONS.md #164), and **it needs no browser**: the catch lives in `Renderer.HandleExceptionViaErrorBoundary`, which is ordinary .NET, so this hosts the real `Renderer`, the real `ErrorBoundary` and real event dispatch. It runs where Playwright is not installable. It pins the finding the whole mapping rests on — a boundary does **not** catch a throw from a handler the parent owns (W1), while it does catch one from a descendant (W2, W1's control). `dotnet run --project tools/error-boundary-oracle`. |
