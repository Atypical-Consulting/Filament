...
## 111. Le record POSITIONNEL entre dans le §5 — la même forme de données qu'un record à corps, écrite plus court ; supersède le refus #77-adjacent

**Décision.** Un record positionnel `record Item(string Name, int Rank)` rejoint le sous-ensemble. Il était
refusé (`unsupported-member` : « un record positionnel porte un ctor/`Equals`/`GetHashCode`/`Deconstruct`
générés dont un module Filament n'a nulle part où mettre »). Ce refus était CONSERVATEUR, pas nécessaire : un
record positionnel est la **MÊME forme de données** qu'un record à corps (`record Item { public string Name
{ get; set; } … }`), écrite plus court, donc il compile vers le MÊME littéral objet `{ name, rank }` — et les
membres générés sont simplement INUTILISÉS (une forme en lecture seule ; le §5 n'admet ni égalité de valeur ni
déconstruction). `Record()` construit les `Props` à partir des paramètres du constructeur primaire ; leur type
est validé comme celui d'une propriété de corps. Suite : **332 tests** (256 générateur / 58 subset / 18
analyzer), runtime 214.

**PREMIÈRE construction d'objet-record en position d'EXPRESSION.** Avant #111, un record ne se construisait QUE
via le repli du site de construction (`Row row = new Row(); row.X = …` → un littéral, dans `ObjectLiteral`),
jamais EN LIGNE. Un record positionnel se construit en ligne (`new Item("alpha", 1)` dans un littéral de liste,
dans `.Add(…)`). `ClassifyExpression` admet donc `ObjectCreationExpression` d'un record LOCAL (`IsLocalRecordCreation`
— un `INamedTypeSymbol { IsRecord: true }` déclaré en source ; tout autre `new` reste refusé), et `Expr()` le
traduit via `RecordLiteral` : arguments positionnels mappés aux props par ORDRE DU CONSTRUCTEUR, assignations
d'initialiseur d'objet mappées par NOM. Le repli du site (chemin PHARE Rows) est INCHANGÉ — le `new Row()` d'un
site n'atteint jamais `Expr()` — donc Rows reste octet pour octet (snapshot vert).

**ALIGNE L'ANALYSEUR ET LE GÉNÉRATEUR.** `ClassifyMember` admet TOUS les records (niveau du KIND), donc
l'analyseur ne signalait déjà PAS un record positionnel — mais le générateur le refusait à un niveau plus
profond (`Record()`). C'était une incohérence latente : `SupportedMembers_ProduceNoDiagnostics` incluait
`private record Row(int Id);` sans diagnostic. #111 rend le générateur cohérent avec l'analyseur.

**UN RECORD PORTANT UNE MÉTHODE RESTE REFUSÉ.** `record Row { … int Twice() … }` (`Code/RecordMember.razor`)
reste refusé : une méthode est du COMPORTEMENT, qu'un littéral objet (une forme de données) ne porte pas. C'est
la ligne architecturale, distincte du raccourci syntaxique que #111 admet. Le témoin `Code/RecordDecl.razor`
bascule refusé → compilé (`PositionalRecord_NowCompiles`).
...
## 116. LINQ (Where/Select/Count/Any/…) sur une List entre dans le §5 — des méthodes de tableau JS ; supersède le refus « BCL hors sous-ensemble »

**Décision.** Les opérateurs LINQ courants sur une `List` rejoignent le §5, mappés sur les méthodes de tableau
JS : `Where`→`filter`, `Select`→`map`, `Count`→`length` (ou `filter(...).length`), `Any`→`some`/`length>0`,
`All`→`every`, `ToList`/`ToArray`→le tableau. Ils étaient refusés (`unsupported-call`), catalogués « BCL, hors
sous-ensemble ». Mais une `List` EST déjà un tableau JS matérialisé (décision rows.js 1), donc les méthodes EAGER
de JS reproduisent EXACTEMENT une chaîne LINQ qui se termine par un scalaire (Count/Any/All) ou par ToList —
vérifié : `[-2,3,-1,5,0].filter(x=>x>0).length` = 2 = `_nums.Where(x=>x>0).Count()`. `LinqInvocation` reconnaît
une méthode dont le type conteneur est `System.Linq.Enumerable` et dispatche ; le récepteur est traduit
récursivement (une chaîne `A.Where(..).Count()` est `Expr(A.Where(..)) + ".length"`). Suite : **353 tests** (275
générateur / 60 subset / 18 analyzer), runtime 214. **LEÇON (6e over-refus corrigé) : « BCL » n'implique pas
« sans mapping fidèle » — LINQ EST des méthodes de tableau.**

**LE LAMBDA PRÉDICAT SE TRADUIT PAR LA MACHINERIE EXISTANTE.** `x => x > 0` (un `SimpleLambdaExpression` avec corps
d'expression) devient `x => x > 0` : `Lambda()` émet `<param> => <Expr(corps)>`, et le corps passe par le MÊME
`Expr` que le reste de @code. Le paramètre `x` est un local ordinaire — `Identifier` résout déjà un
`IParameterSymbol` par son nom, donc `x` reste `x` (jamais un signal). Seul un lambda MONO-paramètre à corps
d'expression est admis (un corps-bloc est refusé).

**UN SOUS-ENSEMBLE ÉTROIT, le reste REFUSÉ.** Seuls Where/Select/Count/Any/All/ToList/ToArray sont dispatchés ;
tout autre opérateur (GroupBy, OrderBy, Join, Aggregate, les surcharges…) tombe sur le refus de `LinqInvocation`.
`Enumerable.Range` (méthode statique, récepteur = un TYPE) est refusé — son récepteur ne s'évalue pas en tableau,
et son résultat est un `IEnumerable` local, refusé à son type (`Code/Linq.razor`). Divulgué. Un `Select`/`Where`
NON terminé (stocké comme `IEnumerable`) est refusé au type de la locale : seules les chaînes terminées par un
scalaire ou `ToList` (dont le type EST dans le §5) traversent.

**GÉNÉRATEUR SEUL, ZÉRO HELPER.** LINQ est des méthodes de tableau JS natives — aucune primitive runtime, aucun
helper émis. `git diff -- src/filament-runtime` vide. MESURÉ (entrée n°35) : `baseline/Linq.Blazor`
...
## 133. JS interop et `@inject` — le pont n'est pas implémenté, il est EFFACÉ

**Décision.** `@inject IJSRuntime JS` entre dans le sous-ensemble, avec `InvokeVoidAsync` et
`InvokeAsync<T>`. Deux non-buts §3 tombent d'un coup — « DI (`@inject`) » et « JsInterop » — et ils tombent
ENSEMBLE parce qu'ils sont la même question : `IJSRuntime` est le seul service qui ait un sens à la
compilation.

**Le lowering.** `await JS.InvokeVoidAsync("localStorage.setItem", "fil", "hello")` →
`await localStorage.setItem('fil', 'hello')`. Blazor résout l'identifiant pointé contre la portée globale du
navigateur **à l'exécution** et sérialise les arguments à travers la frontière .NET/JS. Ce module est déjà de
l'autre côté : l'identifiant est résolu **à la COMPILATION** — vers exactement le même chemin pointé, qui est
du JS légal tel quel — et les arguments sont les expressions traduites elles-mêmes. Il ne reste ni service,
ni appel d'invocation, ni marshalling. Le pont n'est pas implémenté : il est **effacé**.

**`@inject` EST ÉTROIT, ET C'EST TOUTE L'HONNÊTETÉ DE LA TRANCHE.** Un conteneur DI généraliste résout une
implémentation **à l'exécution**, et un module statique n'a aucun conteneur à interroger ; un service écrit
par l'auteur vit dans un `.cs` que ce compilateur ne lit jamais. Ces deux questions sont **refusées, pas
approximées** — `@inject` de toute autre chose donne un `unsupported-directive` qui NOMME ce qui est admis et
POURQUOI. Le témoin `Unsupported/Inject.razor` reste refusé au même emplacement (1,1), motif affiné.

**L'IDENTIFIANT DOIT ÊTRE UN LITTÉRAL**, et c'est le garde porteur : un identifiant CALCULÉ ne pourrait être
résolu qu'en parcourant `globalThis` à l'exécution, c'est-à-dire en embarquant précisément le pont que cette
tranche prétend avoir supprimé. Chaque segment pointé est vérifié comme identifiant JS simple, donc rien qui
ne soit un chemin d'appel ne peut être splicé en position d'appel (témoin `Gate/JsIdentifierComputed.razor`).

**LE SEAM RAZOR EST RESPECTÉ.** `ComponentInjectIntermediateNode` est **internal** à
`Microsoft.AspNetCore.Razor.Language` : impossible de le nommer dans un test de type. La réflexion nécessaire
est donc placée dans `RazorFrontEnd` — avec le reste du seam EOL-Razor (ADR 0001) — et elle **échoue fort** :
si `TypeName`/`MemberName` bougent, c'est la version épinglée qui a changé sous nous, et un résultat vide
signifierait un `@inject` écrit par l'auteur silencieusement ignoré.
**FRONTIÈRES** : `Gate/JsInterop.razor` (IJSRuntime en CHAMP) reste refusé à son type — `Classify` ne connaît
toujours pas ce type, il n'est admis que comme cible d'`@inject` ; le reste de l'API (`InvokeAsync` de module,
`DotNetObjectReference`, streams) est refusé : il existe pour gérer une frontière que ce module n'a pas.
Suite : **441 tests** (363 générateur / 60 subset / 18 analyzer), runtime 214.

**GÉNÉRATEUR SEUL, ZÉRO HELPER.** `git diff -- src/filament-runtime` vide ; runtime gelé à 1 943 o.
MESURÉ (entrée n°52) : `#out` passe de `""` à `"hello"` — une valeur qui ne peut apparaître qu'en écrivant
PUIS relisant le `localStorage` du navigateur — et le contrat vérifie AUSSI `localStorage['fil']` lui-même,
de sorte qu'une implémentation qui truquerait l'affichage sans toucher au stockage serait prise. Identique à
...
## 146. `System.Random` — le hasard semé se PROUVE, le hasard libre se borne

**2026-07-21.** Deuxième valeur non-déterministe après l'horloge (décision 145) — sauf que la moitié de
`Random` n'est PAS non-déterministe : **une instance semée est une séquence exacte**, garantie stable par la
politique de compat de .NET (`new Random(seed)` reste le générateur soustractif de Knuth,
Net5CompatSeedImpl, précisément pour que les séquences semées survivent aux montées de version). Un refus au
motif « non-déterministe » aurait été la sur-refusal conservatrice que ce dépôt a déjà consignée six fois.

**LE MAPPING : une factory émise, deux régimes derrière une interface.** `new Random(s)` → `__rnd(s)` :
le générateur de Knuth réimplémenté EXACTEMENT (MSEED 161803398, table de 55, 4 tours de brassage,
inext/inextp 0/21, le décrément à ==MaxValue, l'échelle Sample 1/int.MaxValue, le chemin grande-plage à
deux tirages) — l'arithmétique reste dans ±2^31, exacte en doubles JS, et l'ordre des opérations
flottantes de `(int)(Sample() * max)` est reproduit à l'identique. `new Random()` et `Random.Shared` →
le régime `Math.random` derrière la MÊME interface (`next/nextTo/nextIn/nextDouble`) : le xoshiro non-semé
de C# n'est pas plus reproductible d'un run à l'autre — la plage et la distribution sont le contrat
observable, et c'est ce que l'oracle borne. `Random.Shared` est UNE instance au niveau module
(`const __rndShared`), la même durée de vie que le singleton statique de C#.

**LA PREUVE EST L'ORACLE CONTRE LE VRAI BCL.** Le témoin sème 42 et rend `sum += rng.Next(1, 7)` : Blazor
exécute le VRAI System.Random, Filament exécute le __rnd émis, et l'égalité OCTET du DOM (5 → 6 → 7, tirages
5, 1, 1 lus sur `dotnet run`, jamais calculés de tête) est la preuve de fidélité de l'algorithme — pas un
test unitaire contre des valeurs recopiées, le BCL lui-même en juge. En sus, un test node rejoue la factory
depuis LES OCTETS ÉMIS et exige la séquence BCL (5 1 1 1434747710).

**FRONTIÈRES NOMMÉES.** `NextBytes` rempli un `byte[]` — pas de maison fidèle pour un buffer d'octets
mutable ; refusé au guichet Random (le buffer est INLINE dans le témoin : un local `byte[]` refuserait à sa
propre déclaration, autre guichet). `NextInt64`/`NextSingle` différés. Et `@rng` — AFFICHER un Random — est
refusé AU SLOT : C# rend « System.Random », JS rendrait « [object Object] » — le rendu-faux-silencieux
que la section 10 interdit. Admission du TYPE single-sourcée dans `TypeSubset.IsRandom` (+
